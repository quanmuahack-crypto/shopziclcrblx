...
const SAFE_SERVICES=new Set(['Trái Ác Quỷ Blox Fruits (Hàng Trade)']);
...
window.setOrderFields=function(name){const safe=SAFE_SERVICES.has(name),robux=isRobux(name),ug=isUG(name),special=robux||ug;const roblox=document.getElementById('robloxAccountField'),pass=document.getElementById('robloxPasswordField'),phone=document.getElementById('robloxPhoneField'),support=document.getElementById('robloxSupportField'),ugfield=document.getElementById('ugphoneAccountField');if(!roblox||!pass||!phone||!support||!ugfield)return;roblox.style.display=ug?'none':'';pass.style.display=(special||safe)?'none':'';support.style.display=(special||safe)?'none':'';phone.style.display='';ugfield.style.display=ug?'':'none';const p=document.getElementById('robloxPassword'),s=document.getElementById('robloxSupportCode');if(safe||special){if(p)p.value='';if(s)s.value='';}};
