(function(){
  ...
  function addGrowAGardenMap1(){
    ...
    catalog['Grow A Garden 2 Map 1']=[
      ['1000 Super Watering Can',110000],
      ['1000 Super Sprinkler',90000],
      ['100 Dragon Breath',120000],
      ['50 Star Fruit',125000],
      ['150 Moon Bloom',189000],
      ['100 Hypno Bloom',218000],
      ['100 Sun Bloom',100000],
      ['100 Super Watering Can',30000],
      ['100 Super Sprinkler',21000],
      ['1 Mega Ice Serpent Rainbow',400000],
      ['Mega Raccoon',110000],
      ['Mega Black Dragon',350000]
    ];
    renderServices();
    setTimeout(applyServiceImages,0);
    setTimeout(applyServiceImages,300);
  ...
  const serviceImages={
    ...
    'Grow A Garden 2 Map 1':MAP1_GAG2_AVATAR
  };
  function applyServiceImages(){
    ...
    const map1Card=[...grid.querySelectorAll('.card')].find(c=>c.querySelector('h3')?.textContent.trim()==='Grow A Garden 2 Map 1');
    if(map1Card&&!map1Card.querySelector('.service-avatar')){
      const img=document.createElement('img');img.className='service-avatar';img.src=MAP1_GAG2_AVATAR;img.alt='Grow A Garden 2 Map 1';img.loading='eager';
      const first=map1Card.querySelector(':scope > div');if(first)first.replaceWith(img);else map1Card.prepend(img);
    }
  }
  function startTop5Guard(){fixedTop5();setTimeout(fixedTop5,500);setTimeout(fixedTop5,1500);setTimeout(fixedTop5,3000)}
  function boot(){addVngService();addAvatarService();addBlackbeardCape();addMap2Gag2();addGrowAGardenMap1();applyServiceImages();setTimeout(applyServiceImages,100);setTimeout(applyServiceImages,600);setTimeout(startTop5Guard,150)}
