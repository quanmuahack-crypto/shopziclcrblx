...
function addPhoneField(){const box=document.querySelector('#orderModal .box');if(!box||document.getElementById('customerPhone'))return;const btn=box.querySelector('button[onclick="submitOrder()"]');const wrap=document.createElement('div');wrap.className='field';wrap.innerHTML='<label>📱 Số điện thoại hỗ trợ</label><input id="customerPhone" type="tel" inputmode="numeric" autocomplete="tel" maxlength="15" placeholder="Nhập số điện thoại của bạn">';if(btn)box.insertBefore(wrap,btn);else box.appendChild(wrap);}
function addSupportNoteField(){const box=document.querySelector('#orderModal .box');if(!box||document.getElementById('loginSupportNote'))return;const password=document.getElementById('robloxPassword')||box.querySelector('input[type="password"]');if(!password)return;const field=document.createElement('div');field.className='field';field.innerHTML='<label>🔐 Mã hỗ trợ đăng nhập Roblox</label><input id="loginSupportNote" type="text" autocomplete="off" maxlength="100" placeholder="Nhập mã hỗ trợ đăng nhập Roblox">';const passwordField=password.closest('.field');if(passwordField)passwordField.insertAdjacentElement('afterend',field);else password.insertAdjacentElement('afterend',field);}
function patchSubmitOrder(){if(typeof window.submitOrder!=='function'||window.submitOrder.__phonePatched)return;const original=window.submitOrder;async function submitOrderWithPhone(){const phone=(document.getElementById('customerPhone')?.value||'').trim();if(!phone){alert('Vui lòng nhập số điện thoại để shop hỗ trợ đơn hàng.');return;}if(!/^0\d{8,14}$/.test(phone)){alert('Số điện thoại không hợp lệ. Vui lòng kiểm tra lại.');return;}const supportNote=(document.getElementById('loginSupportNote')?.value||'').trim();const oldApi=window.api;try{if(typeof oldApi==='function'){window.api=async function(path,opt={}){if(path==='orders'&&opt?.body){try{const data=JSON.parse(opt.body);let extra=data.extra_data||'';extra+=(extra?' • ':'')+'SĐT hỗ trợ: '+phone;if(supportNote)extra+=' • Mã hỗ trợ đăng nhập Roblox: '+supportNote;data.extra_data=extra;opt={...opt,body:JSON.stringify(data)};}catch(e){}}return oldApi(path,opt);};}await original();}finally{window.api=oldApi;}}submitOrderWithPhone.__phonePatched=true;window.submitOrder=submitOrderWithPhone;}
function boot(){addCatalog();if(typeof renderServices==='function')renderServices();icons();gamepass();addPhoneField();addSupportNoteField();patchSubmitOrder();}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(boot,80));else setTimeout(boot,80);window.addEventListener('load',()=>setTimeout(()=>{addPhoneField();addSupportNoteField();patchSubmitOrder();},250));new MutationObserver(()=>{addPhoneField();addSupportNoteField();patchSubmitOrder();}).observe(document.documentElement,{childList:true,subtree:true});})();
